...
fselect(time_period, geographic_level,
        region_name, la_name,
...
ks1_phonics_allSEN <- ks1_phonics %>%
  ...
  summarise(tested_pupils = sum(as.numeric(t_phonics_y1_eligible_pupils), na.rm = T), 
             expected_pupils = sum(as.numeric(t_phonics_y1_met_expected_standard), na.rm = T)) %>%
  mutate(`Percent meeting expected standards in Y1` = 100 * expected_pupils/tested_pupils, 
...
ks4_attainment <- rio::import("data/attainment_ks4/2122_lachar_data.csv") %>% 
  ...
  mutate("Average progress 8 score" = as.numeric(avg_p8score),
...
ks4_destinations <- rio::import("data/destinations_ks4/ks4_dm_ud_202021_la_prov.csv") %>% 
  ...
  pivot_longer(cols = c(all_work, appren, sfc, ssf, fe, other_edu, all_notsust, all_unknown),
               names_to = "destination_raw",
               values_to = "pupils") %>% 
  mutate(`% of pupils` = round(as.numeric(pupils), 2),
...
ks4_destinations_nat <- rio::import("data/destinations_ks4/national/ks4_dm_ud_202021_nat_prov.csv") %>% 
  ...
  mutate(characteristic = recode(characteristic,
   ...
   time_period_to_academic_year() %>% 
  fsubset(academic_year > "2017/18") %>% 
  mutate(appren = as.numeric(appren),
...
ofsted <- ofsted_raw %>%
 # fsubset(LA != "Northamptonshire") %>% # Northamptonshire no longer exists but since W/N Northamptonshire haven't been inspected yet use it anyway
  ...
  mutate(`Publication Date` = lubridate::ymd(`Publication Date`),
         `Visit Date` = lubridate::ymd(`Visit Date`)) %>% #Interpret column as date 
  mutate(`Publication Date` = format(`Publication Date`, "%d %B %Y"),
         `Visit Date` = format(`Visit Date`, "%d %B %Y")) %>% #Reformat to nicer date e.g. 21 June 2022
  
#Now, to clean up the inspection outcomes column
  mutate(`Inspection Outcome` =
           case_when(Type == "Re SEND" & `Written Statement of Action` == "Yes"               ~ "Written Statement of Action (after reinspection)",
   ...
   mutate(`Inspection Outcome` = ifelse(test = la_name == "Tameside" & `Link to visit report` == "https://reports.ofsted.gov.uk/provider/44/80569",
                                        ...
                                        no = `Inspection Outcome`)) %>%  #Verdict was missing from original spreadsheet
 fselect(la_name, region, `Visit Date`,`Publication Date`, `Inspection Outcome`) %>% 
  mutate(box_colour = case_when(`Inspection Outcome` %in% c("Written Statement of Action",
                                                            "Written Statement of Action (after reinspection)") ~ "maroon",
                                `Inspection Outcome` %in% c("No Written Statement of Action",
                                                            "Previous Written Statement of Action Removed (after reinspection)") ~ "green",
...
ehcp_timeliness <- sen2_mi %>% 
  ...
  fselect(time_period, geographic_level, region_code,
         ...
         la_name, NewallEHC, `% of EHCPs issued within 20 weeks` = NoExc20weekRate) %>% 
...
absence_allSEN <- absence %>%
  ...
  fsummarise(sess_overall = sum(as.numeric(sess_overall), na.rm = T), 
...
absence_regional <- rio::import("data/absence/6_absence_2term_characteristics.csv") %>% 
  ...
  mutate(region_name = ifelse(test = region_old %in% c("Inner London", "Outer London"),
                              yes = "London",
                              no = region_old)) %>%  #Collapse Inner/Outer London into one region
  group_by(time_period, characteristic, region_name) %>% 
  summarise(sess_overall = sum(sess_overall),
...
absence_regional_allSEN <- absence_regional %>%
  ...
  summarise(sess_overall = sum(sess_overall),
            sess_possible = sum(sess_possible),
            sess_overall_percent = mean(sess_overall_percent)) %>% # this metric is not apparently used for anything but for consistency and bind_rows...
  ftransform(overall_absence_percent = round(100 * (sess_overall / sess_possible), 5), 
...
years <- append("", sort(c(seq(from = start_year, to = end_year),
                      seq(from = start_year, to = end_year),
                      seq(from = start_year, to = end_year))))

#Create column names by repeating the three categories the required number of times
column_names <- append("la_name",
                       rep(x=c("appealsregistered_", "totalappealabledecisions_", "appealrate_"),
                           times = n_years))
...
tribunals <- tribunals1 %>% 
  ...
  mutate(geographic_level = ifelse(la_name == "England",
                                  yes = "National",
                          no = "Local authority"),
...
tribunals_reg <- tribunals1 %>% 
 ...
 mutate(region_name = ifelse(la_name == "England",
                              yes = "England",
                              no = region)) %>% #Add England as a region
  ...
  pivot_wider(id_cols = c( region_name, year),
              names_from = metric,
              values_from = total_value) %>% #Pivot table wider for computing values
...
destinations_1618 <- rio::import("data/destinations_1618/1618_dm_ud_202021_la_prov.csv") %>% 
  ...
  fselect(time_period, geographic_level, region_name, la_name, characteristic,
          characteristic_group, cohort, all_work, appren, he, fe, other_edu, all_notsust, all_unknown) %>% 
  time_period_to_academic_year() %>% 
  pivot_longer(cols = c(all_work, appren, he, fe, other_edu, all_notsust, all_unknown),
               names_to = "destination_raw",
               values_to = "% of pupils") %>% 
...
destinations_1618_nat <- rio::import("data/destinations_1618/national/1618_dm_ud_202021_nat_rev.csv") %>% 
  ...
  time_period_to_academic_year() %>% 
  mutate(all_notsust = as.numeric(all_notsust),
...
specialist_spend_201819 <- rio::import("data/finance/2021-22-High-Needs-LA-Benchmarking-Tool.xlsx",
  ...
  rowwise() %>% 
  mutate("independent_2018-19" = sum(as.numeric(`specialschools_ind_2018-19`), as.numeric(`ap_ind_2018-19`),
                                      na.rm = TRUE), #add independent topup together for special and AP schools
         ...
         "total_2018-19" = sum(`independent_2018-19`, `state_2018-19`)) %>% #population column - not currently used 
...
specialist_spend_201920 <- rio::import("data/finance/2021-22-High-Needs-LA-Benchmarking-Tool.xlsx",
  ...
  rowwise() %>% 
  mutate("independent_2019-20" = sum(as.numeric(`specialschools_ind_2019-20`), as.numeric(`ap_ind_2019-20`),
                                     na.rm = TRUE),
...
specialist_spend_202021 <- rio::import("data/finance/2021-22-High-Needs-LA-Benchmarking-Tool.xlsx",
  ...
  mutate(la_name = recode(la_name,
                          "Dorset (New, LA code 838)" = "Dorset")) %>% 
  rowwise() %>% 
  mutate("independent_2020-21" = sum(as.numeric(`specialschools_ind_2020-21`), as.numeric(`ap_ind_2020-21`),
                                     na.rm = TRUE),
...
sen_phase_type <- read_csv("data/sen_in_england/sen_phase_type.csv", col_types = c(ehc_plan_percent = "numeric",
                                                                                   sen_support_percent = "numeric"))

percent_pupils_ehcp <- sen_phase_type %>% fsubset(phase_type_grouping == "Total" &
                                          ...
                                          fselect(time_period,
                                                  ...
                                                  ehc_plan_percent,
                                                  sen_support_percent) %>% 
                                        pivot_longer(cols = c(ehc_plan_percent, sen_support_percent),
                                                     names_to = "SEN provision",
                                                     values_to = "% of pupils") %>% 
  time_period_to_academic_year() %>% 
    ftransform(`% of pupils` = round(`% of pupils`, 2),
...
ehcp_ageprofile <- rio::import("data/ehc_plans/sen2_age_caseload.csv") %>%
  ...
  mutate(`Number of EHCPs` = as.numeric(Total_ehc),
    `Age group` = factor(characteristic_age,
                              ordered = TRUE,
                              levels = c("Age 20 to 25",
                                         "Age 16 to 19",
                                         "Age 11 to 15",
                                         "Age 5 to 10",
                                         "Under 5" )))
...
mainstream_with_sen <- sen_phase_type %>%
  fsubset(phase_type_grouping %in% c("State-funded nursery",
                                     "State-funded primary",
                                     "State-funded secondary") &
                                   type_of_establishment == "Total" &
                                   hospital_school == "Total") %>%
  #To combine the nursery, primary and secondary data, first pivot the data to be one row per LA/year
   pivot_wider(id_cols = c(region_name, la_name, time_period, geographic_level),
  ...
  #Add together the three totals for all three school types (using mutate as ftransform doesn't work with rowwise())
   mutate(total_pupils = sum(as.numeric(`total_pupils_State-funded secondary`),
         ...
         total_ehc_plans = sum(as.numeric(`ehc_plan_State-funded secondary`),
                               ...
                               as.numeric(`ehc_plan_State-funded nursery`),
                                 na.rm = TRUE)) %>% 
   ftransform(ehc_percent = 100 * (total_ehc_plans / total_pupils), #Calculate EHC percentage
          sensupport_percent = 100 * (total_sensupport / total_pupils)) %>%  #Calculate SEN support percentage
  fselect(time_period,
          region_name,
          geographic_level,
          la_name,
          ehc_percent,
          sensupport_percent,
          total_pupils) %>%  #Get rid of unused variables
   pivot_longer(cols = c(ehc_percent, sensupport_percent),
               names_to = "SEN provision",
               values_to = "% of pupils") %>% 
  mutate(`SEN provision` = recode(`SEN provision`, ehc_percent = "EHC plan",
...
#Filter to total pupils and pupils in specialist settings
 ...
 left_join(total_headcounts) %>%  #Bring in total headcounts data for calculation of percentages
  ...
  ftransform(total_pupils = as.numeric(total_pupils),
  ...
  ftransform(        all_percent = 100 * (total_pupils / all_headcount), #what % of all pupils go to this provision type
                ehc_plan_percent = 100 * (ehc_plan     / ehc_plan_headcount), #what % of pupils with EHCPs go to this provision type
             sen_support_percent = 100 * (sen_support  / sen_support_headcount)) %>%  # what % of pupils with SEN support go to this provision type
  time_period_to_academic_year() %>% 
  pivot_longer(cols = c(all_percent, ehc_plan_percent, sen_support_percent),
               names_to = "Provision type",
               values_to = "percent") %>% #Bring the three percentages together under "Provision type"
  ...
  select(academic_year, time_identifier,
        ...
        `Provision type`, "% of pupils (with SEN provision type)" = percent, academic_year) %>% 
  fsubset(academic_year >= "2017/18")
...
provider_types_grouped <- provider_types %>% 
   mutate(`Grouped provider type` = ifelse( phase_type_grouping %in% c("Independent school",
                                           "Pupil referral unit",
...
#This combination seems to yield the least missing data (ASD19e for under 18s, ASD19g for over 18s)
  ...
  ftransform(age_group = ifelse(test = BREAKDOWN == "Age Group",
                    ...
                    no =  paste0("Age: ", SECONDARY_LEVEL))) %>% #If the breakdown is Age Group that won't be listed in secondary level, so need to bring it over.
  ftransform(`% with first appointment after more than 13 weeks` = as.numeric(METRIC_VALUE),
...
mentalhealth <- mentalhealth_raw %>%
  mutate(BREAKDOWN2 = ifelse(BREAKDOWN %in% c( "CCG of Residence", "Sub ICB of Residence"),
                              ...
                              no = BREAKDOWN))  %>% #Combine CCG and sub-ICB level, like the NHS PowerBI dashboard does
...
most_recent_mentalhealth_label <- paste0(lubridate::month(max(mentalhealth_ccg$`Year ending`),
                                                  label = TRUE, abbr = FALSE),
                                 " ",
                                 lubridate::year(max(mentalhealth_ccg$`Year ending`)))

year_ago_mentalhealth_label <- paste0(lubridate::month(max(mentalhealth_ccg$`Year ending`) ,
                                                  label = TRUE, abbr = FALSE),
                                 " ",
...
eng_mentalhealth <- mentalhealth %>% fsubset(BREAKDOWN == "England") %>% compare_to_previous(`Number of children and young people`, `Year ending`)
eng_absence <- absence_regional %>% fsubset(region_name == "England") %>% compare_to_previous(`Overall absence %`, time_period, characteristic)
...
eng_autism <- autism %>% fsubset(BREAKDOWN == "Age Group") %>% compare_to_previous(`% with first appointment after more than 13 weeks`, date, nhs_name)
